<script>
  ...
  import WorkWithUsForm from '../components/forms/WorkWithUsForm.svelte';
  import { loadRouteComponent, normalizeRoutePath } from './routing/routes.js';

  let currentPath = typeof window !== 'undefined' ? normalizeRoutePath(window.location.pathname) : '/';
  let RoutePage = null;
  ...
  async function loadRoutePage(path) {
    ...
    const loader = () => loadRouteComponent(path);
  ...
  async function handleNavbarNavigate(path, options = {}) {
    ...
    const currentBrowserPath = normalizeRoutePath(window.location.pathname);
